# GEMINI.md - Spectre Project Context

## Project Overview
Spectre is a high-performance, cross-platform Neovim GUI frontend built with C++20. It leverages native GPU rendering (Vulkan on Windows, Metal on macOS) for low-latency text and grid updates.
...
### Running the App
- **Windows:** `.\build\Debug\spectre.exe` (or `.\r.bat` for a quick build & run)
- **macOS:** `./build/spectre` (or `./r.sh`)
...
## Testing
Spectre includes a suite of native tests covering grid logic, RPC parsing, input handling, and renderer state.

- **Run Tests:** Use `t.bat` (Windows) or `./t.sh` (macOS).
- **Underlying Tool:** CTest is used as the test runner.
- **Integration Test:** Includes a `spectre-app-smoke` test that verifies the app can spawn Neovim and reach a "flushed" UI state within a timeout.
...
- **Libraries:**
  - `spectre-types`: Shared PODs, events, and logging.
  - `spectre-window`: SDL3 window abstraction.
  - `spectre-renderer`: Public API and platform-specific backends.
  - `spectre-font`: Font loading, shaping, and glyph caching.
  - `spectre-grid`: Cell-based UI state and highlights.
  - `spectre-nvim`: Process management and msgpack-RPC protocol.

### Coding Style
- **Standard:** C++20.
- **Naming:** `snake_case` for variables and functions, `PascalCase` for classes and enums.
- **Logging:** Use the `SPECTRE_LOG_<LEVEL>(category, ...)` macros.
...
### Git Workflow
- Always verify changes against the `spectre-app-smoke` test if `nvim` is available.
...
- `libs/spectre-renderer/include/spectre/renderer.h`: Public rendering interface.
- `libs/spectre-nvim/src/ui_events.cpp`: Processes Neovim UI redraw events.

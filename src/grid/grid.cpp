...
void Grid::scroll(int top, int bot, int left, int right, int rows) {
    ...
    if (left == 0 && right == cols_) {
        // Full-width scroll: use memmove for efficiency
        if (rows > 0) {
            // Scroll up: move rows [top+rows, bot) to [top, bot-rows)
            ...
            // Scroll down: move rows [top, bot+rows) to [top-rows, bot)
            int shift = -rows;
            int src = top * cols_;

...
bool VkContext::create_render_pass() {
    ...
    color_attachment.format = VK_FORMAT_B8G8R8A8_UNORM; // Will be updated after swapchain creation
...
bool VkContext::recreate_swapchain(int width, int height) {
    ...
    // Recreate render pass with correct format if needed
    if (render_pass_ != VK_NULL_HANDLE && swapchain_.swapchain != VK_NULL_HANDLE) {
        // Render pass format matches - just rebuild swapchain
    }
    ...
    // If render pass format doesn't match, recreate it
    // For simplicity, recreate render pass each time
    if (render_pass_ != VK_NULL_HANDLE) {
        vkDestroyRenderPass(device_, render_pass_, nullptr);
        render_pass_ = VK_NULL_HANDLE;
    }

    // Create render pass with actual swapchain format
    VkAttachmentDescription color_attachment = {};

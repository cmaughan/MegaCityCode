#include "renderer/vulkan/vk_atlas.h"
...
bool VkAtlas::initialize(VkContext& ctx) {
    ...
    // Swizzle R channel to all RGB channels so white text renders correctly
    ...
    // Staging buffer (large enough for full atlas)
    ...
    // Transition to shader read layout initially (with cleared data)
...
void VkAtlas::upload_region(VkContext& ctx, int x, int y, int w, int h, const uint8_t* data) {
    // Copy row by row into staging buffer (data may not be contiguous with atlas width)

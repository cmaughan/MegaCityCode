...
bool VkRenderer::initialize(IWindow& window) {
    ...
    // Initialize grid buffer (start with 80x24)
    ...
    // Sync objects, command buffers, descriptors
...
void VkRenderer::update_cells(std::span<const CellUpdate> updates) {
    for (const auto& u : updates) {
        ...
        // Shader does: pos_y + size_y - offset_y, we want: pos_y + ascender - bearing_y
...
bool VkRenderer::begin_frame() {
    ...
    // Safe to restore cursor now - GPU is done reading this frame's SSBO
    restore_cursor();
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        // Must recreate descriptor pool since layouts changed
        if (desc_pool_) {
    ...
    if (needs_descriptor_update_) {
        // Only update current frame's descriptors - the other frame may still be in flight
        update_descriptor_sets_for_frame(current_frame_);
        // Mark that other frames still need updating
...
void VkRenderer::record_command_buffer(VkCommandBuffer cmd, uint32_t image_index) {
    ...
    if (total_cells > 0) {
        // Push constants: screen size and cell size
        float push_data[4] = {
        ...
        vkCmdDraw(cmd, 6, bg_instances, 0, 0); // 6 vertices per quad (2 triangles)
...
void VkRenderer::apply_cursor() {
    ...
    if (cursor_shape_ == CursorShape::Block) {
        ...
        // Bar/underline: use the extra overlay cell at the end of the SSBO
        int overlay_idx = grid_cols_ * grid_rows_; // one past the grid
        GpuCell overlay = {};
...
void VkRenderer::restore_cursor() {
    ...
    // Only block cursor modifies the grid cell; bar/underline use the overlay
    int idx = cursor_row_ * grid_cols_ + cursor_col_;
...
void VkRenderer::end_frame() {
    ...
    // Don't restore cursor in SSBO here - GPU is still reading it.
    // Restore happens at the start of begin_frame after fence wait.

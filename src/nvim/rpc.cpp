...
bool NvimRpc::initialize(NvimProcess& process) {
    process_ = &process;
    read_buf_.resize(1024 * 1024); // 1MB read buffer
    running_ = true;
...
MpackValue NvimRpc::request(const std::string& method, const std::vector<MpackValue>& params) {
    ...
    // Serialize: [0, msgid, method, params]
    char buf[65536];
    mpack_writer_t writer;
    mpack_writer_init(&writer, buf, sizeof(buf));

    mpack_start_array(&writer, 4);
    mpack_write_u32(&writer, 0); // request type
...
void NvimRpc::notify(const std::string& method, const std::vector<MpackValue>& params) {
    ...
    mpack_write_u32(&writer, 2); // notification type
    mpack_write_str(&writer, method.c_str(), (uint32_t)method.size());
...
void NvimRpc::reader_thread_func() {
    ...
    while (running_) {
        ...
        // Try to parse complete messages from accumulated data
        while (!accum.empty()) {
...
MpackValue NvimRpc::read_value(mpack_reader_t* reader) {
    ...
    case mpack_type_ext: {
        // Neovim uses ext types for Buffer, Window, Tabpage handles
        uint32_t len = mpack_tag_ext_length(&tag);
        // Read and discard ext data, store as integer if small enough
        if (len <= 8) {
            char ext_data[8] = {};
            mpack_read_bytes(reader, ext_data, len);
            // Interpret as integer (neovim ext types are typically small ints)
            int64_t ext_val = 0;

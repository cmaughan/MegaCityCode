...
// Decode a UTF-8 string to get the first codepoint
...
// Check if a codepoint is likely double-width (CJK, etc.)
...
void UiEventHandler::process_redraw(const std::vector<MpackValue>& params) {
    for (size_t ei = 0; ei < params.size(); ei++) {
        ...
        // "flush" has no arguments — just the name
        if (name == "flush") {
            if (on_flush) on_flush();
            continue;
        }

        // Each event may have multiple argument sets
...
void UiEventHandler::handle_grid_line(const MpackValue& args) {
    ...
    int grid_id = (int)args.array_val[0].as_int(); // Usually 1 (default grid)
    ...
    // args[3] is array of cells: [text, hl_id?, repeat?]
    ...
    for (auto& cell : cells) {
        ...
        // Empty string means this is the second cell of a double-width char
        if (text.empty()) {
            col++;
            continue;
        }
        ...
        for (int r = 0; r < repeat; r++) {
            ...
            col++;
            if (dw) col++; // Skip continuation cell
        }
    }
}

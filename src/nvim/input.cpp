...
std::string NvimInput::translate_key(int keycode, uint16_t mod) {
    ...
    // On Windows, SDL_KMOD_GUI is the Windows key
    result += key + ">";
...
void NvimInput::on_key(const KeyEvent& event) {
    ...
    // Special keys always go through translate
    std::string translated = translate_key(keycode, mod);

    if (!translated.empty()) {
        // If we're sending a special key with Ctrl/Alt, suppress the following text input
        suppress_next_text_ = (mod & (SDL_KMOD_CTRL | SDL_KMOD_ALT)) != 0;
        send_input(translated);
    } else {
        // Regular key - will be handled by on_text_input unless it has modifiers
...
void NvimInput::on_text_input(const TextInputEvent& event) {
    ...
    // Handle '<' specially to avoid vim notation confusion
    if (text == "<") {
        send_input("<lt>");
    } else {
        send_input(text);

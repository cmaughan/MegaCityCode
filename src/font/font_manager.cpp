...
bool FontManager::initialize(const std::string& font_path, int point_size, float display_ppi) {
    ...
    // FT_Set_Char_Size takes size in 1/64th of a point and DPI.
    // FreeType computes: device_pixels = point_size * dpi / 72
    // This means the same point_size produces the same physical height
    // on any display, regardless of pixel density.
...
bool FontManager::set_point_size(int point_size) {
    ...
    // Recreate HarfBuzz font for new size

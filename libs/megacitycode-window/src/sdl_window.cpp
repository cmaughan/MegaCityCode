...
{
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "Display / DPI diagnostics begin");
    ...
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "SDL logical size: %d x %d", lw, lh);
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "SDL pixel size: %d x %d", pw, ph);
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "SDL display scale: %.3f", sdl_scale);
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "SDL effective DPI: %.1f", 96.0f * sdl_scale);
    ...
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "CG logical pixels: %zu x %zu", cgLogicalW, cgLogicalH);
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "CG physical pixels: %zu x %zu", cgPhysicalW, cgPhysicalH);
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "CG physical size: %.1f x %.1f mm", physicalSize.width, physicalSize.height);
    if (physicalSize.width > 0)
    {
        ...
        MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "Computed PPI (logical): %.1f", ppi_logical);
        MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "Computed PPI (physical): %.1f", ppi_physical);
    }
    ...
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "SDL content scale: %.3f", contentScale);
#endif
    ...
    if (hwnd)
    {
        ...
        MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "MDT_EFFECTIVE_DPI: %u x %u", eff_x, eff_y);
        MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "MDT_ANGULAR_DPI: %u x %u", ang_x, ang_y);
        MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "MDT_RAW_DPI: %u x %u", raw_x, raw_y);
        ...
        MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "Monitor logical rect: %d x %d (%s)",
            mon_lw, mon_lh,
        ...
        {
            MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "Monitor device pixels: %lu x %lu",
            ...
            if (hdc)
            {
                ...
                DeleteDC(hdc);
                MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "GetDeviceCaps HORZSIZE/VERTSIZE: %d x %d mm", mm_w, mm_h);
                MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "GetDeviceCaps HORZRES/VERTRES: %d x %d", log_px_w, log_px_h);
                MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "GetDeviceCaps DESKTOPHORZRES/VERTRES: %d x %d", desk_px_w, desk_px_h);
                MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "GetDeviceCaps LOGPIXELSX: %d", logpx_dpi);
                if (mm_w > 0)
                {
                    float phys_ppi = dm.dmPelsWidth / (mm_w / 25.4f);
                    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "Computed physical PPI (DEVMODE/mm): %.1f", phys_ppi);
                }
            }
        }
    }
#endif
    MEGACITYCODE_LOG_DEBUG(LogCategory::Window, "Display / DPI diagnostics end");
}
...
bool SdlWindow::initialize(const std::string& title, int width, int height)
{
    ...
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Window, "SDL_Init failed: %s", SDL_GetError());
        return false;
    }
    ...
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Window, "SDL_RegisterEvents failed: %s", SDL_GetError());
        return false;
    }
    ...
    if (!window_)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Window, "SDL_CreateWindow failed: %s", SDL_GetError());
        return false;
    }
...
bool SdlWindow::wait_events(int timeout_ms)
{
    ...
    if (!got_event)
    {
        ...
        if (err && err[0] != '\0')
        {
            MEGACITYCODE_LOG_WARN(LogCategory::Window, "SDL wait failed: %s", err);
        }
        return true;
    }

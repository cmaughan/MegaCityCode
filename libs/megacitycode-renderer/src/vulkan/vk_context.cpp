...
bool VkContext::initialize(SDL_Window* window)
{
    ...
    auto inst_ret = instance_builder
                        .set_app_name("megacitycode")
                        .set_engine_name("megacitycode")
    ...
    if (!inst_ret)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create Vulkan instance: %s", inst_ret.error().message().c_str());
        return false;
    }
    ...
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create Vulkan surface: %s", SDL_GetError());
        return false;
    }
    ...
    if (!phys_ret)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to select physical device: %s", phys_ret.error().message().c_str());
        return false;
    }
    ...
    if (!dev_ret)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create logical device: %s", dev_ret.error().message().c_str());
        return false;
    }
    ...
    if (!queue_ret)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to get graphics queue");
        return false;
    }
    ...
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create VMA allocator");
        return false;
    }
...
bool VkContext::create_render_pass(VkFormat format, VkRenderPass& render_pass)
{
    ...
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create render pass");
        return false;
    }
...
bool VkContext::build_swapchain_resources(int width, int height, PendingSwapchainResources& pending)
{
    ...
    if (!sc_ret)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create swapchain: %s", sc_ret.error().message().c_str());
        return false;
    }
    ...
    if (!images_ret)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to get swapchain images: %s", images_ret.error().message().c_str());
        destroy_pending_swapchain_resources(pending);
        return false;
    }
    ...
    if (!views_ret)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to get swapchain image views: %s", views_ret.error().message().c_str());
        destroy_pending_swapchain_resources(pending);
        return false;
    }
...
bool VkContext::create_framebuffers(SwapchainInfo& swapchain, VkRenderPass render_pass)
{
    ...
    for (size_t i = 0; i < swapchain.image_views.size(); i++)
    {
        ...
        {
            MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create framebuffer");

...
bool MetalRenderer::ensure_capture_buffer(size_t width, size_t height)
{
    ...
    if (!buffer)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create Metal capture buffer");
        return false;
    }
...
bool MetalRenderer::initialize(IWindow& window)
{
    ...
    if (!device)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create Metal device");
        return false;
    }
    ...
    if (!metalView)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create Metal view: %s", SDL_GetError());
        return false;
    }
    ...
    if (!library)
    {
        MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to load shader library: %s",
    ...
    // Create background pipeline
    {
        ...
        {
            MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to find bg shader functions");
            return false;
        }
        ...
        if (!pipeline)
        {
            MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create bg pipeline: %s",
    ...
    // Create foreground pipeline
    {
        ...
        if (!vertFunc || !fragFunc)
        {
            MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to find fg shader functions");
            return false;
        }
        ...
        if (!pipeline)
        {
            MEGACITYCODE_LOG_ERROR(LogCategory::Renderer, "Failed to create fg pipeline: %s",
                [[error localizedDescription] UTF8String]);
            return false;
        }
    ...
    MEGACITYCODE_LOG_INFO(LogCategory::Renderer, "Metal renderer initialized (%s)", [[device name] UTF8String]);
    return true;
}

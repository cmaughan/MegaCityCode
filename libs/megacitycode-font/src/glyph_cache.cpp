...
bool bitmap_to_rgba(const FT_Bitmap& bmp, std::vector<uint8_t>& out, bool& is_color)
{
    ...
    MEGACITYCODE_LOG_WARN(LogCategory::Font, "Unsupported FreeType pixel mode: %u", bmp.pixel_mode);
    return false;
}
...
bool GlyphCache::reserve_region(int w, int h, int& atlas_x, int& atlas_y, const char* label)
{
    ...
    {
        MEGACITYCODE_LOG_WARN(LogCategory::Font, "Atlas full, cannot fit %s (%dx%d)", label, w, h);
        overflowed_ = true;
        return false;
    }

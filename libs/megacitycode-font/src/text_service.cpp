...
struct TextService::Impl
{
    ...
    bool initialize(int point_size, float ppi)
    {
        ...
        MEGACITYCODE_LOG_INFO(LogCategory::Font, "Primary font path: %s", font_path.c_str());
    ...
    AtlasRegion resolve_cluster(const std::string& text)
    {
        ...
        atlas_reset_pending = true;
        atlas_reset_count++;
        MEGACITYCODE_LOG_WARN(LogCategory::Font, "Glyph atlas reset after exhaustion (count=%d)", atlas_reset_count);
    ...
    void initialize_fallback_fonts()
    {
        ...
        for (const auto& path : candidates)
        {
            ...
            fallback.shaper.initialize(fallback.font.hb_font());
            MEGACITYCODE_LOG_INFO(LogCategory::Font, "Fallback font loaded: %s", path.c_str());
        }
    }

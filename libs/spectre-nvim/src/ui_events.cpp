...
void UiEventHandler::process_redraw(const std::vector<MpackValue>& params)
{
    for (size_t ei = 0; ei < params.size(); ei++)
    {
        ...
        if (event.type != MpackValue::Array || event.array_val.empty())
            continue;
...
{
    if (!grid_ || args.type != MpackValue::Array || args.array_val.size() < 4)
        return;
    ...
    for (auto& cell : cells)
    {
        if (cell.type != MpackValue::Array || cell.array_val.empty())
            continue;
...
{
    if (args.type != MpackValue::Array || args.array_val.size() < 3)
        return;
...
{
    if (!grid_ || args.type != MpackValue::Array || args.array_val.size() < 7)
        return;
...
{
    if (!highlights_ || args.type != MpackValue::Array || args.array_val.size() < 2)
        return;
...
{
    if (!highlights_ || args.type != MpackValue::Array || args.array_val.size() < 3)
        return;
...
{
    if (args.type != MpackValue::Array || args.array_val.size() < 2)
        return;
...
void UiEventHandler::handle_option_set(const MpackValue& args)
{
    ...
    if (on_option_set)
        on_option_set(name, args.array_val[1]);
}

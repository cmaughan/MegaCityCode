...
MpackValue NvimRpc::request(const std::string& method, const std::vector<MpackValue>& params)
{
    ...
    if (!process_->write(reinterpret_cast<uint8_t*>(buf), len))
    {
...
void NvimRpc::write_value(mpack_writer_t* writer, const MpackValue& val)
{
    ...
    case MpackValue::Bool:
        mpack_write_bool(writer, val.bool_val);
        break;
    case MpackValue::Int:
        mpack_write_i64(writer, val.int_val);
        break;
    case MpackValue::UInt:
        mpack_write_u64(writer, val.uint_val);
        break;
    case MpackValue::Float:
        mpack_write_double(writer, val.float_val);
        break;
